...
class PasteHandler extends Extension {
  ...
  constructor(opts) {
      super(opts);
         ...
         */
        this.forcePlainText = opts.forcePlainText ? opts.forcePlainText : true;

        /* cleanPastedHTML: [boolean]
         * cleans pasted content from different sources, like google docs etc.
         */
        this.cleanPastedHTML = opts.cleanPastedHTML ? opts.cleanPastedHTML : false;

        /* cleanReplacements: [Array]
         * custom pairs (2 element arrays) of RegExp and replacement text to use during paste when
         * __forcePlainText__ or __cleanPastedHTML__ are `true` OR when calling `cleanPaste(text)` helper method.
         */
        this.cleanReplacements = opts.cleanReplacements ? opts.cleanReplacements : [];

        /* cleanAttrs:: [Array]
         * list of element attributes to remove during paste when __cleanPastedHTML__ is `true` or when
         * calling `cleanPaste(text)` or `pasteHTML(html, options)` helper methods.
         */
        this.cleanAttrs = opts.cleanAttrs ? opts.cleanAttrs : ['class', 'style', 'dir'];

        /* cleanTags: [Array]
         * list of element tag names to remove during paste when __cleanPastedHTML__ is `true` or when
         * calling `cleanPaste(text)` or `pasteHTML(html, options)` helper methods.
         */
        this.cleanTags = opts.cleanTags ? opts.cleanTags : ['meta'];
